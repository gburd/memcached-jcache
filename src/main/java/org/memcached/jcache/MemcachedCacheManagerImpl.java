...
public class MemcachedCacheManagerImpl implements MemcachedCacheManager {
    ...
    public synchronized MemcachedClient getMemcachedClient(String cacheName) {
        ...
        boolean sharedClient = Boolean
            .valueOf(properties.getProperty(cacheName + ".useSharedClientConnection", "false"));
        if (sharedClient) {
            client = clients.get("");
        }
        if (client == null) {
            ...
            if (StringUtils.isBlank(servers)) {
                ...
                addresses = AddrUtil.getAddresses(servers)
                    .stream()
                    .filter(
                            address -> serviceAvailable(address.getAddress().getHostAddress(), address.getPort()))
                    ...
                    client.addObserver(
                            new ConnectionObserver() {
                                ...
                                public void connectionEstablished(SocketAddress socketAddress, int i) {
                                    LOG.info(
                                            String.format(
                                                    "Connection to MemcacheD established: %s%s.",
                                                    socketAddress.toString(),
                                                    i > 0 ? String.format(" on reconnect # %d", i) : ""));
                                }
    ...
    private void validateConfiguration(CompleteConfiguration<?, ?> configuration) {
        ...
        if (!(expiryPolicy instanceof EternalExpiryPolicy)
            ...
            throw new UnsupportedOperationException(
                    "Invalid expiry policy " + expiryPolicy.getClass().getSimpleName() + " configuration!");
        }
